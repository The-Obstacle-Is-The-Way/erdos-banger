...
def get_functions(path: Path) -> Iterator[tuple[str, int, int]]:
    """
    Yield (function_name, start_line, line_count) for all functions in a file.
    ...
    for node in ast.walk(tree):
        if (
            ...
            yield (node.name, node.lineno, lines)
...
def check_function_exemption(
    path_str: str, func_name: str, path: Path, lineno: int
) -> tuple[bool, str | None]:
    """
...
def audit_functions(base_path: Path) -> list[FunctionViolation]:
    ...
    for pattern in patterns:
        for path in base_path.glob(pattern):
            ...
            path_str = str(path.relative_to(base_path))
            for func_name, lineno, lines in get_functions(path):
                if lines > FUNCTION_LOC_THRESHOLD:
                    exempted, debt_id = check_function_exemption(
                        path_str, func_name, path, lineno
                    )
                    violations.append(
                        FunctionViolation(
                            path=path_str,
...
def print_report(
    ...
    if module_violations:
        ...
        print("-" * 70)
        for v in sorted(module_violations, key=lambda x: -x.lines):
            status = "⚠️  EXEMPT" if v.exempted else "❌ VIOLATION"
            debt_info = f" ({v.debt_id})" if v.debt_id else ""
            print(f"  {status}{debt_info}")
            print(f"    {v.path}: {v.lines} LOC (threshold: {v.threshold})")

    if function_violations:
        print("\n🔧 FUNCTION SIZE VIOLATIONS")
        print("-" * 70)
        for v in sorted(function_violations, key=lambda x: -x.lines):
            status = "⚠️  EXEMPT" if v.exempted else "❌ VIOLATION"
            debt_info = f" ({v.debt_id})" if v.debt_id else ""
            print(f"  {status}{debt_info}")
            print(
                f"    {v.path}:{v.lineno} {v.name}(): "
                f"{v.lines} LOC (threshold: {v.threshold})"
            )
